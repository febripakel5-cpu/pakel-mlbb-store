...
# =====================================================================================
#  OFFICIAL PAKEL MLBBSTORE - MASTER ULTIMATE v6 (SIMPLE QRIS + FULL FITUR)
# =====================================================================================
...
# =====================================================================================
#  KONFIGURASI QRIS — MODE SIMPLE (1 GAMBAR UNTUK SEMUA NOMINAL)
# =====================================================================================
# Isi dengan direct image URL (.jpg / .png) ATAU file_id Telegram.
# Contoh direct URL : "https://i.ibb.co/xxxx/qris-pakel.jpg"
# Contoh file_id    : "AgACAgUAAxkBAAExxxxx..."
...
def roll_lucky_draw():
    labels = [h[0] for h in LUCKY_DRAW_HADIAH]
    weights = [h[3] for h in LUCKY_DRAW_HADIAH]
    pilihan = random.choices(LUCKY_DRAW_HADIAH, weights=weights, k=1)[0]
    return pilihan
...
def _apply_referral_bonus_if_eligible_helper(new_buyer_chat_id):
    try:
        rows = []
        changed = False
        referrer_to_bonus = None        with open(F_REFERRALS, "r") as f:
            for line in f:
                parts = line.strip().split('|')
                if len(parts) == 4:
    ...
    try:
        ...
        if data == 'menu_utama':
            ...
            # ==== QRIS (1 GAMBAR + NOMINAL BULAT) ====
                ...
                # Hitung harga final (asli atau promo, TANPA kode unik)
                ...
                # Hapus pesan pilihan metode sebelumnya
                try:
                    bot.delete_message(chat_id=chat_id, message_id=message_id)
                except Exception:
                    pass

                # Kirim FOTO QRIS dengan caption invoice lengkap
                try:
                    bot.send_photo(
                        chat_id=chat_id,
                    ...
                    # Fallback: kalau foto gagal, kirim teks saja
                    bot.send_message(
                        chat_id=chat_id,
...
def handle_text_and_reviews(message):
    ...
    # Cek voucher manual    upper_txt = txt.upper().strip()
...
print("[INFO] Pakel MlbbStore v6 SIMPLE-QRIS Edition Berhasil Dijalankan...")
