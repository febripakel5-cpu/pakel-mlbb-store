...
# =====================================================================================
#  OFFICIAL PAKEL MLBBSTORE - MASTER ULTIMATE SECURITY & REFERRAL EDITION
# =====================================================================================
...
QRIS_CUSTOM_LINKS = {
    65000: QRIS_WEB_LINK, 75000: QRIS_WEB_LINK, 95000: QRIS_WEB_LINK,
    100000: QRIS_WEB_LINK, 120000: QRIS_WEB_LINK, 150000: QRIS_WEB_LINK,
    200000: QRIS_WEB_LINK, 250000: QRIS_WEB_LINK,
}
...
# =====================================================================================
#  BAGIAN 4: SISTEM REFERRAL & GENERATOR KODE UNIK
# =====================================================================================
...
def _apply_referral_bonus_if_eligible_helper(new_buyer_chat_id):
    try:
        ...
        if referrer_to_bonus:
            add_user_points(referrer_to_bonus, 10, "Bonus referral (teman belanja)")
            try:
                bot.send_message(referrer_to_bonus,
                                 "🎉 <b>BONUS REFERRAL CAIR!</b>\nTemanmu berhasil belanja pertama kali. +10 Poin masuk ke saldo loyalitasmu!",
                                 parse_mode="HTML")
            except Exception:
                pass
    except FileNotFoundError:
...
def _kirim_poin_referral(chat_id, message_id, l='id'):
    ...
    text = (
        ...
        "💡 <i>Ajak teman pakai link ini, kamu dapat +10 Poin otomatis saat mereka sukses belanja pertama kali!</i>"
    )
    markup = types.InlineKeyboardMarkup()
...
def callback_handler_master(call):
    ...
    try:
        # --- 1. ADMIN ACTIONS (ACC / TOLAK / APOIN / TPOIN) ---
        ...
        # --- 2. GENERAL & USER NAVIGATION ACTIONS ---
        if data == 'menu_utama':
            user_points = get_user_points(chat_id)
            markup = types.InlineKeyboardMarkup(row_width=1)
            markup.add(
            ...
            text = (f"🔥 {greeting}, Kak {user.first_name}!\n"
                    f"🪙 Saldo Poin Anda: <b>{user_points} Poin</b>\n\n"
...
print("[INFO] Pakel MlbbStore Master Ultimate 8-Part Edition Berhasil Dijalankan...")
